...
const createSignToken = (user) => {
  return jwt.sign(
    ...
    "rewer-wkr-ewew-werkewrlewkrlewkr-few",
    {
      expiresIn: "90d",
    }
  );
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!token) {
    return next(
      new AppError(
        "You are not logged in, please log in first! or email is incorrect",
        401
      )
    );
  }
  ...
  const decode = await promisify(jwt.verify)(
    token,
    "rewer-wkr-ewew-werkewrlewkrlewkr-few"
  ); //this will give us decoded payload object which in this case contains user id
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(existingPassword, user.password))) {
    return next(
      new AppError("You have entered the wrong existing password!", 401)
    );
  }
